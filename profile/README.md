...
**GAIA (Goal-oriented Autonomous Intelligence for Adaptive GUI Testing)**는 '기획서'를 이해하여 테스트 전략을 스스로 수립하고, 코드와 이미지를 결합한 하이브리드 검증 방식으로 앱의 품질을 검사하는 차세대 AI 테스트 에이전트입니다.
...
기획서(PDF)를 분석하여 테스트 시나리오를 AI가 직접 생성합니다. 더 이상 깨지기 쉬운 스크립트를 사람이 직접 작성하고 유지보수할 필요가 없습니다.
...
GAIA는 'MCP-엣지' 분산 아키텍처를 따릅니다:

- **MCP (두뇌 🧠)**: 무거운 AI 연산(기획서 분석, 전략 수립)을 담당하는 중앙 서버
- **엣지 (손발 🦾)**: 사용자 PC에 설치되는 데스크톱 에이전트로, MCP로부터 명령을 받아 실제 브라우저를 제어하고 결과를 보고

이 구조는 사용자 PC의 리소스를 최소한으로 사용하면서, 강력한 AI의 분석 능력을 최대한으로 활용하기 위해 설계되었습니다.

## 📖 사용법

1. **기획서 업로드**: PDF 형태의 기획서를 업로드합니다.
